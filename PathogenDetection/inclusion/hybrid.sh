...
do
	...
	echo $sample
	 ~/krakenuniq/krakenuniq --report-file /data3/jaebeom/benchmarks/covid_inclusion/results/ensemble/${sample}_kru_report.tsv \
	 	--paired \
	 	--db /data3/jaebeom/benchmarks/covid_inclusion/db_with_human/krakenuniq \
	 	--threads 16 \
	 	${read1} \
	 	${read2} \
	...
	~/kraken2/kraken2 --db /data3/jaebeom/benchmarks/covid_inclusion/db_with_human/kraken2x \
		--paired \
	...
	~/dev/Metabuli/build-release/src/metabuli binning2report \
		...
		/data3/jaebeom/benchmarks/covid_inclusion/db_with_human/krakenuniq/taxonomy
done
